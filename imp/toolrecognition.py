...
def update_tools_for_frames(frame, modframe, tools, errors, drawerLocation,timestamp,drawer,configuration,classifier, record):
    ...
    for tool in updatedTools["Tools"]:
        ...
        if visible:
            countours =remove_from_contours(countours, toolLocation,drawerLocation,frame)
            ...
            status = is_checked_out(crop,modframe,tool,toolLocation,gcon.get("thresholdtool"),gcon.get("thresholdsymbol"),gcon.get("degrees"),gcon.get("degreesdiv"),errors,timestamp,drawer["ID"],record)
            print(errors)
...
def calculate_location(tool,drawer,drawerLocation):
    ...
    print(toolSize)
    return toolSize
def remove_from_contours(contours, toolLocation,drawerLocation,frame):
...
def is_checked_out(image, modFrame, tool, toolLocation, threshold,thresholdSymbol, degrees, degreesDiv, errors, timeStamp,drawerID,record):
    ...
    if similarityTool >= similarityNoTool:
        ...
        place = placeTool
        if (notEntireVisible or tool["ClassifierThinks"] ==None or toolType == tool["ClassifierThinks"]) and similarityTool != 0 and symbol_check(tool,toolLocation,image,modFrame,thresholdSymbol,degrees,degreesDiv,record):
            text = tool["Name"] + " " +  str(round(similarityTool,2)) +'%'
            checkedOut =1
            color = (256,0,256)
        else:
            place = ((0,0),(toolLocation[2],toolLocation[3]))
            text = "error" + str(tool["ID"]) +" " +toolType+  str(round(similarityTool,2)) +'%'
            if tool["error"]!= 1:
    ...
    if record ==1:
        ...
        (wt, ht), _ = cv2.getTextSize(
         text, cv2.FONT_HERSHEY_SIMPLEX, .005*toolLocation[2], 5)
  
        modFrame = cv2.rectangle(modFrame, (toolLocation[0]+place[0][0], toolLocation[1]+place[0][1] - ht), (toolLocation[0]+place[0][0] + wt, toolLocation[1]+place[0][1]), (255, 0,0), 3)
        cv2.putText(modFrame, text, (toolLocation[0]+place[0][0], toolLocation[1]+place[0][1]),cv2.FONT_HERSHEY_SIMPLEX,.005*toolLocation[2], (36,255,12), 3)
        cv2.rectangle(modFrame, (toolLocation[0]+place[0][0],toolLocation[1]+place[0][1]), 
                      (toolLocation[0] +place[0][0]+ toolLocation[2],toolLocation[1] +place[0][1]+ toolLocation[3]), color, 3 )
...
def classifier_check(classifier, image):
    ...
    print(labels[index])
    return labels[index]
def symbol_check(tool, toolLocation, image, modFrame, threshold, degrees, degreesDiv, record):
